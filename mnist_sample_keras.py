...
def load_imaegs(image_list,label_list,data_type):
    ...
    for dir in os.listdir("./mini/"+data_type):
        if dir == ".DS_Store":
            continue
    
        dir1 = "./mini/"+data_type+"/" + dir 
        label = 0
    
        if dir == "dogs":    # dogはラベル0
            label = 0
        elif dir == "cats": # catはラベル1
            label = 1
...
def load_model(model_dir,nametag):
    model_filename = 'cnn_model'+str(nametag)+'.json'
    weights_filename = 'cnn_model_weights'+str(nametag)+'.hdf5'
...
def save_model(model,model_dir,epoch):
    ...
    open(os.path.join(model_dir,'cnn_model'+str(epoch)+'.json'), 'w').write(json_string)
    yaml_string = model.to_yaml()
    open(os.path.join(model_dir,'cnn_model'+str(epoch)+'.yaml'), 'w').write(yaml_string)
    print('save weights')
    model.save_weights(os.path.join(model_dir,'cnn_model_weights'+str(epoch)+'.hdf5'))
...
for f in glob.glob('./mini/test/*/*.jpg'):
    print('**********************************************************')
    print(f)
    start = time.time()
